...
@app.route("/api/manager/pms/<review_id>/share", methods=["POST"])
@token_required
...
def checkin_photo():
    ...
    if employee_shift == "morning":
    ...
    else:  # night shift check-in: 4:30 PM – 8:00 PM (late from 7:00 PM)
        ...
        if current_time < TIME_1630:
            ...
            return jsonify({
                "message": "Check-in closed for Night Shift (window ended 8:00 PM)."
            }), 400
        day_type = "full"
...
def _send_leave_notification(leave_doc: dict, employee: dict):
    ...
    try:
        ...
        # Leaves this employee applied in the current calendar month (with status breakdown)
        ...
        try:
            ...
            # Build "2 applied · 1 Approved · 1 Pending" style string
            parts = []
            if status_counts.get("Approved"):
                parts.append(f"{status_counts['Approved']} Approved")
            if status_counts.get("Pending"):
                parts.append(f"{status_counts['Pending']} Pending")
            if status_counts.get("Rejected"):
                parts.append(f"{status_counts['Rejected']} Rejected")
        ...
        monthly_detail = (
            f"{monthly_leave_count} applied · {monthly_breakdown}"
...
# =============================================================================
# ACHIEVEMENTS  (Employee of Month / Quarter / Year, awards, salary hikes)
# =============================================================================
...
@app.route("/api/admin/achievements/<achievement_id>", methods=["DELETE"])
@token_required
